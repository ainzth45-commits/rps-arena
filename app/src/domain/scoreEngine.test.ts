...
describe("เรทคะแนนพื้นฐาน (spec §8.1)", () => {
  it("ผู้เล่นเลือกเป้าเอง: +4 / +1 / −3", () => {
    expect(playerDeltaTenths("win", false, 1, cfg)).toBe(40);
    expect(playerDeltaTenths("draw", false, 0, cfg)).toBe(10);
    expect(playerDeltaTenths("lose", false, 0, cfg)).toBe(-30);
  });

  it("ผู้เล่นกดสุ่ม: +5 / +1 / −2", () => {
    expect(playerDeltaTenths("win", true, 1, cfg)).toBe(50);
    expect(playerDeltaTenths("draw", true, 0, cfg)).toBe(10);
    expect(playerDeltaTenths("lose", true, 0, cfg)).toBe(-20);
  });

  it("ผู้ท้าชิง: +3 / +1 / −2", () => {
    expect(challengerDeltaTenths("win", cfg)).toBe(30);
    expect(challengerDeltaTenths("draw", cfg)).toBe(10);
    expect(challengerDeltaTenths("lose", cfg)).toBe(-20);
  });
...
describe("โบนัสสตรีค (spec §8.2)", () => {
  ...
  it("ตารางในสเปกต้องตรงเป๊ะ — เลือกเอง (ฐาน 4)", () => {
    expect(playerDeltaTenths("win", false, 1, cfg)).toBe(40); // 4.0
    expect(playerDeltaTenths("win", false, 2, cfg)).toBe(44); // 4.4
    expect(playerDeltaTenths("win", false, 3, cfg)).toBe(48); // 4.8
    expect(playerDeltaTenths("win", false, 5, cfg)).toBe(56); // 5.6
    expect(playerDeltaTenths("win", false, 10, cfg)).toBe(76); // 7.6
  });

  it("ตารางในสเปกต้องตรงเป๊ะ — กดสุ่ม (ฐาน 5)", () => {
    expect(playerDeltaTenths("win", true, 1, cfg)).toBe(50); // 5.0
    expect(playerDeltaTenths("win", true, 2, cfg)).toBe(55); // 5.5
    expect(playerDeltaTenths("win", true, 3, cfg)).toBe(60); // 6.0
    expect(playerDeltaTenths("win", true, 5, cfg)).toBe(70); // 7.0
    expect(playerDeltaTenths("win", true, 10, cfg)).toBe(95); // 9.5
  });

  it("ตัวคูณไม่ใช้กับเสมอ และไม่ใช้กับการหักคะแนนตอนแพ้", () => {
    expect(playerDeltaTenths("draw", false, 9, cfg)).toBe(10);
    expect(playerDeltaTenths("lose", false, 9, cfg)).toBe(-30);
    expect(playerDeltaTenths("lose", true, 9, cfg)).toBe(-20);
  });

  it("ไม่มีเพดานสตรีค", () => {
    expect(playerDeltaTenths("win", false, 21, cfg)).toBe(120); // 4 × 300% = 12.0
  });

  it("ทุกผลลัพธ์เป็นจำนวนเต็ม (ไม่มีทศนิยม float หลุดมา)", () => {
    for (let streak = 1; streak <= 50; streak++) {
      expect(Number.isInteger(playerDeltaTenths("win", false, streak, cfg))).toBe(true);
      expect(Number.isInteger(playerDeltaTenths("win", true, streak, cfg))).toBe(true);
    }
  });
});
...
describe("สถานการณ์จริง — ปั้นสตรีค 3 ครั้งติดแล้วแพ้", () => {
  it("คะแนนสะสมถูกต้องทุกก้าว", () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      score = applyDelta(score, playerDeltaTenths("win", false, streak, cfg));
    }
    ...
    score = applyDelta(score, playerDeltaTenths("lose", false, streak, cfg));
    ...
    streak = nextStreak(streak, "win");
    expect(playerDeltaTenths("win", false, streak, cfg)).toBe(40);
  });
});
